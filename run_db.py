...
class RunRecorder(object):
    def __init__(self):
        ...
        self.attrs['argv'] = [
            os.path.relpath(arg) if os.path.exists(arg) else arg
            for arg in sys.argv]

        branches = subprocess.check_output('git branch', shell=True).decode()
        ...
        log = subprocess.check_output(
            'git log --format=oneline', shell=True).decode()
        ...
        diff_stat = subprocess.check_output(
            'git diff HEAD --stat', shell=True).decode()
        if diff_stat.strip():
            self.attrs['clean'] = False
            self.attrs['diff_stat'] = diff_stat
            diff = subprocess.check_output('git diff HEAD', shell=True).decode()
